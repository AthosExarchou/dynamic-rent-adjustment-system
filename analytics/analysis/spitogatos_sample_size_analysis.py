...
OUTPUT_PLOT = Path("outputs/spitogatos_sample_funnel.png")
...
# Spitogatos Scraper Validation Rules
...
def main():
    ...
    print("Loading Spitogatos population data...")
    ...
    print(f"True weighted mean of population: {true_mean:.4f} €/m²")
    ...
    plt.axhline(
        ...
        color='#D0021B',
        linewidth=2,
        label=f"True Mean ({true_mean:.2f} €/m²)"
    )
    ...
    plt.plot(
        ...
        linestyle=":",
        color='#F5A623',
        label="Ascending Price Bias (Cheapest First)"
    )
    ...
    plt.plot(
        ...
        linestyle=":",
        color='#7ED321',
        label="Descending Bias (Luxury First)"
    )

    plt.xlabel("Sample Size (N) - Number of Listings Scraped")
    plt.ylabel("Weighted Average Price (€/m²)")
    plt.title("Spitogatos Market: Sample Size Stability & Bias Analysis")
    plt.legend()
